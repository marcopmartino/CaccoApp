...
class _InvitePage extends State<InvitePage> {
  ...
  Widget build(BuildContext context) {
    void checkIfUserIsInGroup(String groupId) async {
      final userGroups = await UsersNetwork.getUserGroupsList(FirebaseAuth.instance.currentUser!.uid);
      for (var group in userGroups) {
        if (group == groupId) {
          if (context.mounted) {
            Navigation.replaceWithoutAnimation(context, GroupDetailsPage(groupId: groupId));
          }
        }
      }
    }
    checkIfUserIsInGroup(widget.groupId);
    return Scaffold(
        backgroundColor: AppColors.lightBlack,
        appBar: Utils.getAppbarInvite(context, "Dettagli gruppo"),
                                      ...
                                      child: ElevatedButton(
                                        onPressed: () {
                                          ...
                                          floatingSnackBar(
                                              ...
                                              context: context,
                                              duration: const Duration(seconds: 5));
                                          Navigation.replaceWithoutAnimation(context, GroupDetailsPage(groupId: widget.groupId));
                                        },
                                      ...
                                      child: ElevatedButton(
                                        onPressed: () {
                                          ...
                                          floatingSnackBar(
                                              ...
                                              context: context,
                                              duration: const Duration(seconds: 5));
                                          Navigation.replaceFromBottom(context, const HomePage());
                                        },
